...
import com.chengxusheji.po.Info;
...
import com.chengxusheji.service.YejiService;
...
public class ProjectController extends BaseController {
    ...
    @Resource YejiService yejiService;
...
//	添加业绩
	@RequestMapping(value="/yeji",method=RequestMethod.POST)
	public void yeji(@RequestBody String yeji,HttpServletResponse response) throws Exception {
		com.alibaba.fastjson.JSONObject jsonObject = JSON.parseObject(yeji); 
	    String data=jsonObject.get("data").toString();
		System.out.println(data);
		List<Yeji> lists=JSON.parseArray(data, Yeji.class); 
		for(Yeji yeji1:lists) {
			UserInfo userInfo=userInfoService.queryUserName(yeji1.getUser_name());
			yeji1.setDept_name(userInfo.getDepartment());
			yeji1.setUserObj(userInfo.getUser_name());
			yejiService.insertById(yeji1);
		}
		 String message = "项目添加成功!";
    ...
    public String seacrh(HttpServletRequest r,HttpServletResponse response,Model model) throws Exception {
	  ...
	  Set<String> set=new HashSet();
	   response.setContentType("text/json;charset=UTF-8");
			...
			return "Project/project_frontUserAdd";
    }
	/*客户端ajax方式提交添加项目信息*/
	...
	public void userAdd(Project project,Info info, BindingResult br,
	...
	public String userFrontlist(@ModelAttribute("projectTypeObj") ProjectType projectTypeObj,String projectName,String mainPerson,String otherPerson,String startDate,@ModelAttribute("userObj") UserInfo userObj,String shzt,String shr,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (projectName == null) projectName = "";
		if (mainPerson == null) mainPerson = "";
		if (startDate == null) startDate = "";
	...
	public void OutToExcel(@ModelAttribute("projectTypeObj") ProjectType projectTypeObj,String projectName,String mainPerson,String startDate,@ModelAttribute("userObj") UserInfo userObj,String shzt,String shr, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "项目id","项目类型","项目名称","项目主图","负责人","参与人员","业绩点","项目来源","科研代号","开始日期","结束日期","科研资金","提交用户","审核状态"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<projectList.size();i++) {
        	Project project = projectList.get(i); 
        	dataset.add(new String[]{project.getProjectId() + "",project.getProjectTypeObj().getProjectTypeName(),project.getProjectName(),project.getProjectPhoto(),project.getMainPerson(),project.getOtherPerson(),project.getPoint(),project.getComeFrom(),project.getKydh(),project.getStartDate(),project.getEndDate(),project.getKyMoney() + "",project.getUserObj().getName(),project.getShzt()});
        }
        /*
