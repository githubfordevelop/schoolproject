				...
				if ($("#projectEditForm").form("validate")) {
					...
					if(!$("#projectEditForm").form("validate")) {
						...
						$("#projectEditForm").form({
						    url:"Project/" + $("#project_projectId_edit").val() + "/managerUpdate",
						    onSubmit: function(){
								if($("#projectEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
				...
				$.ajax({
					...
					success : function (project, response, status) {
						...
						if (project) { 
							...
							$("#project_otherPerson_edit").validatebox({
								required : true,
								missingMessage : "",
							});
							$("#project_point_edit").val(project.point);
							$("#project_point_edit").validatebox({
								required : true,
